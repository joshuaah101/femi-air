<div class="bg-white px-5 py-4 shadow-lg rounded">
    <section class="flex justify-between items-center border-b-2 py-3">
        ...
        <div class="flex justify-between items-center space-x-8 font-medium font-mono text-lg">
                    <span
                        class="{{ $current_step==='flight'  ? 'border-b-2 border-red-500' : '' }} cursor-pointer py-2"
                        wire:click="moveTo('flight')">Flight selection</span>
            <span
                class="{{  $current_step==='ticket' ? 'border-b-2 border-red-500' : '' }} cursor-pointer py-2"
                wire:click="moveTo('ticket')">Passenger information</span>
            <span class="{{  $current_step==='summary' ? 'border-b-2 border-red-500' : '' }} cursor-pointer py-2"
                  wire:click="moveto('summary')">Preview and confirmation</span>
            <span class="{{  $current_step==='payment' ? 'border-b-2 border-red-500' : '' }} cursor-pointer py-2"
                  wire:click="moveto('payment')">Payment</span>
        </div>
    </section>
    <div class="container">
                    ...
                    <tr>
                        <td class="px-6 py-4 whitespace-no-wrap border-b border-blue-200 flex justify-center flex-col items-center">
                            ...
                            <p class="mt-5 text-red-500 text-xs font-medium">
                                * Duration : {{$item->landing_at->diffInHours($item->departure_at)}} hour(s) non-stop
                            </p>
                        </td>
                        @if($item->cabin)
                            @foreach($item->cabin as $cabin)
                                {{--  //check number of seats empty --}}
                                @if(isset($cabin->seats) && ($cabin->seats()->whereHas('booking')->count()!== $cabin->seats()->count()))
        ...
        @if($current_step ==="payment")
            ...
            <section class="mt-8 px-32">
                ...
                <section class="flex justify-between mt-5">
                    <button type="button" wire:click="moveTo('flight')"
